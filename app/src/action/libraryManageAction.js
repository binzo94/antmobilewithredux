...
export function amountRequest(){
	return {
		type:MANAGE.LIBRARY_AMOUNT_REQUEST,
		isFetching:true,
	}
}
...
export function amountSuccess(data){
    return {
        type:MANAGE.LIBRARY_AMOUNT_SUCCESS,
        isFetching:false,
        data
    }
}
...
export function amountFailed(message){
    return {
        type:MANAGE.LIBRARY_AMOUNT_FAILED,
        isFetching:false,
        message
    }
}
...
export function getAmount(){
    return (dispatch) =>{
        ...
        return fetchAsyncGet(PATH.BASEPATH+'getEndTotalAmount.do','').then((res) => {
            if (res.status === 200) {
                return res.json();
            }
            if(res.status === 503){
