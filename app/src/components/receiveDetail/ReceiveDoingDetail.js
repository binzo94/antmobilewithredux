...
class ReceiveDoingDetail extends React.Component {
    constructor(props) {
        super(props);
        this.timer=null;
        this.state = {
                            ...
                            <List className="my-list">
                                {
                                    this.props.cardListData.archivesDoc.length !== 0? this.props.cardListData.archivesDoc.map((item, index) => {
                                        if(item.exist=="true")
                                            return (
                                            <List.Item key={index} onClick={this.callPhoneBox.bind(this,item.DOCNAME,item.DOCID,PATH.FILEURL+"schema="+localStorage.getItem('schema')+"&type=1&ID="+item.DOCID)}>
                                                {item.DOCNAME}
                                            </List.Item>)
                                        else
...
export default connect(mapStateToProps,mapDispatchToProps)(ReceiveDoingDetail);
