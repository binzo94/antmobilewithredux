...
import {updateIsPublicInfo} from '../../action/detailAction.js';
...
class AuditOpen extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            reviewsteps: [],   //审核步骤
    ...
    onChange  = ( name, value) => {
        this.setState({
           ...
           // this.props.updateIsPublicInfo(this.props.cardListData.ARCHIVESID,value);
           /* this.props.setShowUser(users);
            ...
            this.props.setGljIsPublic(value);
        });
        this.setState({
            modal: false
        });
    ...
    spanStyle(){//页面显示
        ...
        if(!this.state.radio.name){
            this.props.stepNames.map((item, index) => {
                if(item.name==this.props.cardListData.IS_PUBLIC){
                    this.setState({
                        radio: {
                            tasks: item.tasks,
                            name: item.name,
                    ...
                    >
                        <List>
                            {this.props.stepNames.length !== 0?this.props.stepNames.map((item, index) => {
                                ...
                                return (
                                    <RadioItem key={index} checked={this.state.radio.name === item.value } onChange={() => this.onChange(item.name, item.value)}>
                                          {item.name}
                                    </RadioItem>
                                );
                            }):"结束"}
...
export default connect(mapStateToProps,mapDispatchToProps)(AuditOpen);
