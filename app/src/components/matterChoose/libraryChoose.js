...
import LibraryList from '../matterList/LibraryList';
import {getEndMatters, matterChooseHasmore, matterChooseNofetech} from '../../action/matterChooseAction.js';
...
function mapDispatchToProps(dispatch){
  return {
  	...
  	getEndMatters:(processType,pageSize,pageIndex,isquery)=>dispatch(getEndMatters(processType,pageSize,pageIndex,isquery))
  };
}

class libraryChoose extends React.Component{
  constructor(props){
      super(props);
      this.state = {
  ...
  loadData(){
    this.props.getEndMatters(this.state.processType,this.state.pageSize,this.state.pageIndex,true);
  }
    goBack(){
    ...
    myscroll=()=>{
        ...
        if(pageHeight-viewportHeight-scrollHeight<20&&this.props.isloading==false){
            ...
            this.props.getEndMatters(this.state.processType,this.state.pageSize,pagenum,false);
        }
        }
    ...
    insertBlock=()=> {
        ...
        if ( this.props.data !== undefined&& this.props.data.length!==0) {
            return this.props.data.map((item,index)=>{
                return <LibraryList {...this.state} key={index} data={item} title={this.state.title}/>
            })
        }
        ...
        let message = (
            <div className='matterChoose'>
              <NavBar leftContent="返回"
                  ...
                  >
                  {this.state.processType=='0'?'发文库':'收文库'}
              </NavBar>
...
export default connect(mapStateToProps,mapDispatchToProps)(libraryChoose);
